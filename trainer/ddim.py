...
class Learner(BaseLearner):
    ...
    def sample(self, bs, seed, labels):
        ...
        if self.model_args.method == 'ensemble':
            original_dict = deepcopy(self.unet.state_dict())
            model_path = os.path.join(self.training_args.all_dirs[task_id], 'model.pth')
            if os.path.exists(model_path):
                self.unet.load_state_dict(torch.load(f'{self.training_args.all_dirs[task_id]}/model.pth', map_location=self.unet.device))
                print(labels, task_id, model_path)
        # if self.model_args.method == 'ensemble' and task_id != self.data_args.task_id:
        #     self.unet.load_state_dict(torch.load(f'{self.training_args.all_dirs[task_id]}/model.pth', map_location=self.unet.device))
        ...
        if self.model_args.method == 'ensemble':
            self.unet.load_state_dict(original_dict)
        return image
